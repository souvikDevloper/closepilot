...
type UploadKey = 'payments' | 'invoices' | 'settlements' | 'bank_transactions' | 'ground_truth';
...
export default function ClosePilotClient({ initialResult }:{ initialResult:ReconciliationResult }) {
  ...
  const runRequest = async (payload:unknown,label:string) => {
    ...
    try {
      const response=await fetch('/api/reconcile',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':`ui-${Date.now()}`},body:JSON.stringify(payload)});
      ...
      notify(`${number(data.metrics.input_records)} records processed · ${data.metrics.matched_pairs} pairs verified`);
    } catch(error) {
  ...
  const runUploads=()=>{
    ...
    runRequest({payments:uploads.payments,invoices:uploads.invoices,settlements:uploads.settlements,bank_transactions:uploads.bank_transactions,ground_truth:groundTruth?.length?groundTruth:undefined},'Uploaded judge-style batch');
  };
  ...
  const exportCsv=()=>{
    const header='record_id,source,reference,amount,status,confidence,reason,action\n';
    const escape=(value:string|number)=>`"${String(value).replaceAll('"','""')}"`;
    const body=allRecords.map((record)=>[record.id,record.source,record.reference,record.amount,record.status,record.confidence,record.reason,record.action].map(escape).join(',')).join('\n');
