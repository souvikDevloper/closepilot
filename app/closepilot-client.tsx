...
const REPRODUCIBLE_BENCHMARK_THROUGHPUT = 12_421;
...
export default function ClosePilotClient({ initialResult }:{ initialResult:ReconciliationResult }) {
  ...
  const hasGroundTruth = metrics.ground_truth_pairs > 0;
  ...
  const throughputDetail = timingAvailable
    ? 'engine records / second on this run'
    : roundTripMs === null
      ? 'local 132,500-row reproducible run'
  ...
  const runRequest = async (payload:unknown,label:string) => {
    if (running) return;
    setRunning(true); setRunStep(0); setUploadError(''); setRoundTripMs(null); setSummaryResponse(false);
    ...
    try {
      ...
      const measuredRoundTrip=Math.max(1,Date.now()-started);
      const remaining=Math.max(0,850-(Date.now()-started));
      if(remaining) await new Promise((resolve)=>window.setTimeout(resolve,remaining));
      setResult(data); setDatasetName(label); setRoundTripMs(measuredRoundTrip); setSummaryResponse(Boolean(data.truncated)); setRunStep(3); setView('overview');
  ...
  const runUploads=()=>{
    ...
    const uploadTotal=([uploads.payments,uploads.invoices,uploads.settlements,uploads.bank_transactions,uploads.settlement_recon_items]).reduce((sum,rows)=>sum+(rows?.length??0),0);
    runRequest({payments:uploads.payments,invoices:uploads.invoices,settlements:uploads.settlements,bank_transactions:uploads.bank_transactions,settlement_recon_items:uploads.settlement_recon_items,ground_truth:groundTruth?.length?groundTruth:undefined,response_mode:uploadTotal>20_000?'summary':'full'},uploadTotal>20_000?'Uploaded large batch · summary response':'Uploaded judge-style batch');
  };
    ...
    <section className="main-panel" id="workspace"><header className="topbar"><button className="mobile-brand brand-button" onClick={()=>go('overview')}><span className="brand-mark">C</span> ClosePilot</button><div className="crumb"><span>Workspace</span><b>/</b> {viewLabel}</div><div className="top-actions"><a className="github-shortcut" href={PUBLIC_REPOSITORY} target="_blank" rel="noreferrer">GitHub ↗</a><span className="live-pill"><i/> API live</span><button className="api-shortcut" onClick={()=>go('data')}>POST /api/v1/reconcile</button></div></header>
      <div className="content">
        {view==='overview'&&<>
          ...
          <section className="proof-strip" aria-label="Measured run proof"><span><b>{number(metrics.processed_records)}</b> primary + evidence rows</span><i/><span><b>{timingAvailable?`${metrics.duration_ms!.toFixed(2)} ms`:'n/a'}</b> engine timing</span><i/><span><b>{percent(metrics.precision)}</b> precision</span><i/><span><b>{percent(metrics.false_auto_match_rate)}</b> false auto-match</span><i/><span><b>{metrics.silent_drops}</b> silent drops</span></section>
        ...
        {(view==='records'||view==='exceptions')&&<><section className="page-heading"><div><div className="eyebrow">{view==='records'?`${number(decisionCount)} EXPLAINED DECISIONS`:`${number(metrics.exception_records)} FAIL-CLOSED RECORDS`}</div><h1>{view==='records'?'Every decision has evidence.':'Uncertainty never becomes money movement.'}</h1><p>{view==='records'?(summaryResponse?'Large-run metrics cover the complete batch; this table is a bounded response sample.':'Search the measured output from the actual reconciliation engine.'):'Review ambiguity, missing evidence and schema failures without forced matches.'}</p></div><button className="secondary-button" onClick={exportCsv}>↓ Export response CSV</button></section><section className="mini-stats"><div><span>Matched records</span><b>{number(metrics.matched_records)}</b><em>{percent(metrics.input_resolution_rate)} of all inputs</em></div><div><span>Needs review</span><b>{number(metrics.review_records)}</b><em className="amber-text">human gate</em></div><div><span>Blocked</span><b>{number(metrics.blocked_records)}</b><em className="red-text">no write</em></div><div><span>F1 score</span><b>{percent(metrics.f1)}</b><em>{hasGroundTruth?'measured':'truth not supplied'}</em></div></section><section className="work-card ledger-card"><div className="filterbar"><label className="searchbox"><span>⌕</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Search record or reference"/></label><select value={source} onChange={(event)=>setSource(event.target.value)} aria-label="Filter by source">{sources.map((item)=><option key={item}>{item}</option>)}</select><span className="result-count">{number(filtered.length)} loaded</span></div><MatchTable rows={filtered.slice(0,view==='exceptions'?40:50)} onOpen={setSelected}/><div className="card-footer"><span>Showing {Math.min(filtered.length,view==='exceptions'?40:50)} of {number(filtered.length)} loaded records{summaryResponse?' · complete metrics retained':''}</span><span className="footer-proof">{result.ruleset} · checksum {result.checksum}</span></div></section></>}
